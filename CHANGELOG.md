...
- **The reference recording wiring no longer lets its sink decide a run's outcome.** `examples/recording-example.js` awaited the sink inside `onRun`, so a sink that threw, such as one calling `JSON.stringify` on a trace holding a circular HTTP client error, made `runEffect` reject instead of returning the flow's own `Failure`. An error from `keep` or the sink now goes to a new `onSinkError` option, which defaults to `console.error`.
